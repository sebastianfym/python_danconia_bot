...
def processing_name_handlers_id_list_func(city_name) -> list:
    body_req = ReqApi()
...
def description_commands(message) -> None:
...
def send_picture(message, max_count_pic, hotel_id) -> None:
    body_req = ReqApi()
    counter = 0
...
def bestdeal_funcs_body_work(city_exists, returned_all_hotels_list, message, body_req, max_hotels, min_price, max_price,
                             permissible_range, check_picture, max_count_pic, hotels_id_list, days_between_dates) -> None:
